...
from api.claps.schemas import ClapRequest, ClapResponse
from api.claps import service as clap_service
...
def clap(
    ...
    result = clap_service.add_clap(
        session, article_id, data.count, user_id=user_id, ip=ip
    )
    return result
...
def get_claps(
    ...
    total = clap_service.get_total_claps(session, article_id)

    if current_user:
        user_claps = clap_service.get_user_claps(session, article_id, current_user.id)
    else:
        ip = get_client_ip(request)
        user_claps = clap_service.get_ip_claps(session, article_id, ip)
